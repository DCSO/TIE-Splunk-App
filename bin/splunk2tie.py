...
def query_tie(ioc_value):
    ...
    request = urllib.request.Request("{}?value={}".format(str(tie_args['feed_api']), ioc_value))
...
def main():
    ...
    if len(sys.argv) != 14:
        print(
            "Usage: python splunk2tie.py [value] [id] [data_type] [categories] [min_severity] [max_severity] [min_confidence] [max_confidence] [updated_at] [last_seen] [families] [actors] [hotness]")
        sys.exit(1)
