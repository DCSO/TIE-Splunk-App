...
def query_subscription(bloomfile):
    if bloomfile['bloomfilter']['subscr_id'] == "":
        ...
        response = json.loads(urllib.request.urlopen(post).read())
...
def get_results(bloomfile, sub_id):
    tie_args = cli.getConfStanza('dcso_hunt_setup', 'tie')
    TIE_TOKEN = str(tie_args["token"])
    request = urllib.request.Request("{}/{}/data/latest".format(bloomfile['bloomfilter']['bf-res_api'], sub_id))
    ...
    sys.stdout.write(urllib.request.urlopen(request).read())
...
def main():
    ...
    if os.path.exists(os.path.join(os.path.dirname(__file__), '../local/bloom.json')):
        ...
        data = {"bloomfilter": {"subscr_id": "", "bf-res_api": "https://tie.dcso.de/api/v1/stored-query-results",
                                "query_id": "f978be4f-1274-4f6e-8cf9-60cc3d943bdd",
                                "bf-sub_api": "https://tie.dcso.de/api/v1/stored-query-subscriptions"}}
